...
class MarvelModelWrapper(mlflow.pyfunc.PythonModel):
    def load_context(self, context):
        # Chuyển đổi dấu xược ngược (\) của Windows thành xược xuôi (/) của Linux container
        path_a = str(context.artifacts["sklearn-pipeline-model-A"]).replace("\\", "/")
        path_b = str(context.artifacts["sklearn-pipeline-model-B"]).replace("\\", "/")

        self.model_a = mlflow.sklearn.load_model(path_a)
        self.model_b = mlflow.sklearn.load_model(path_b)
...
with mlflow.start_run() as run:
    ...
    mlflow.pyfunc.log_model(
        ...
        signature=signature,
        pip_requirements=["mlflow", "scikit-learn", "pandas", "numpy", "lightgbm"],
    )
